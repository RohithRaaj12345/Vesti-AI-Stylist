...
def build_measurement_prompt(measures) -> str:
    ...
    """
    return (
        "Add a clean GEOMETRIC BODY-PROPORTION analysis overlay on top of this photo, "
        "in the minimalist style of a professional stylist's body-frame map.\n\n"
        "ABSOLUTE RULES:\n"
        "- Keep the underlying photo EXACTLY the same: same person, face, hair, body, "
        "clothing, pose, background, lighting, framing and dimensions. Do NOT redraw, "
        "restyle, move or alter the photo or the person — only add thin overlay lines "
        "on top.\n"
        "- Use ONE single colour for every line, mark and label: thin, slightly "
        "translucent WHITE. No other overlay colours.\n"
        "- All lines must be thin, perfectly straight and sharp (technical-drawing "
        "quality), aligned precisely to the body.\n\n"
        "Draw these overlay elements:\n"
        "- A vertical CENTRE AXIS line straight down the middle of the body, from the "
        "top of the head to between the feet.\n"
        "- A horizontal AXIS line across the torso at waist level, spanning the body width.\n"
        "- Right-angle corner BRACKETS marking the SHOULDER width at the two shoulders, "
        "and corner brackets marking the HIP width at the two hips.\n"
        "- Two diagonal lines from each shoulder down toward the opposite hip, crossing "
        "at the body centre to form an hourglass / X triangulation.\n"
        "- A small DIAMOND (rhombus) outline at the centre where the diagonals cross "
        "(around the waist/navel).\n\n"
        "Then add small, neat WHITE sans-serif measurement labels beside the matching "
        "lines, using the EXACT text below (do not overlap the body or other labels):\n"
        f"- Height (beside the vertical axis): {measures.estimated_height}\n"
        f"- Shoulders (near the shoulder brackets): {measures.shoulder_width}\n"
        f"- Chest: {measures.chest}\n"
        f"- Waist (near the horizontal axis): {measures.waist}\n"
        f"- Hips (near the hip brackets): {measures.hip}\n"
        f"- Inseam (along the lower leg): {measures.inseam}\n"
        f"- Arm (along one arm): {measures.arm_length}\n\n"
        "The result must look neat, minimal and precise — a single white geometric "
        "overlay on the completely unchanged photo."
    )
